    ...
    public class PartCover22Parser : ParserBase
    {
        ...
        private Class ProcessClass(Assembly assembly, string className)
        {
            var fileIdsOfClass = this.types
                .Where(type => type.Attribute("asm").Value.Equals(assembly.Name) && type.Attribute("name").Value.Equals(className))
                .Elements("method")
                .Elements("code")
                .Elements("pt")
        ...
        private CodeFile ProcessFile(Class @class, string filePath)
        {
            ...
            var seqpntsOfFile = this.types
                .Where(type => type.Attribute("asm").Value.Equals(@class.Assembly.Name) && type.Attribute("name").Value.StartsWith(@class.Name, StringComparison.Ordinal))
                .Elements("method")
                .Elements("code")
                .Elements("pt")
