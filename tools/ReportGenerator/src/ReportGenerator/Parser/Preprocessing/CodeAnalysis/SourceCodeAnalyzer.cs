...
namespace Palmmedia.ReportGenerator.Parser.Preprocessing.CodeAnalysis
{
    ...
    internal static class SourceCodeAnalyzer
    {
        ...
        /// <summary>
        /// Searches the given <see cref="ICSharpCode.NRefactory.Ast.INode">INodes</see> recursivly for classes.
        /// </summary>
        ...
        private static IEnumerable<TypeDeclaration> FindClasses(IEnumerable<AstNode> nodes)
        {
            ...
            foreach (var node in nodes)
            {
                ...
                if (@class != null || node is NamespaceDeclaration || node is CompilationUnit)
                {
        ...
        /// <summary>
        /// Gets the topmost <see cref="ICSharpCode.NRefactory.Ast.INode"/> in the given source file.
        /// </summary>
        /// <param name="filename">The filename.</param>
        /// <returns>The topmost <see cref="ICSharpCode.NRefactory.Ast.INode"/> in the given source file.</returns>
            ...
            else
            {
                try
                {
                    using (var fs = new FileStream(filename, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        var compilationUnit = parser.Parse(fs, filename);

                        if (parser.HasErrors || compilationUnit == null)
                        {
                            return null;
                        }
